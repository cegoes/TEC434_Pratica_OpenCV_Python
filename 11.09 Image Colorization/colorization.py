...
caminho = Path('11.09 Image Colorization')

numpy_file = np.load(str(caminho / 'pts_in_hull.npy'))
Caffe_net = cv.dnn.readNetFromCaffe(str(caminho / "models/colorization_deploy_v2.prototxt"), str(caminho / "models/colorization_release_v2.caffemodel"))
...
class Window(Frame):
    ...
    def uploadImage(self):
        ...
        cv.imwrite(str(caminho / "result.png"), (bgr_output*255).astype(np.uint8))

    def color(self):

        load = Image.open(str(caminho / "result.png"))
        load = load.resize((480, 360), Image.ANTIALIAS)
